                ...
                <div>
                  <div className="flex items-center gap-2">
                    <h1 className="text-2xl font-black text-gray-800 dark:text-white tracking-tight transition-colors">
                      {user ? (user.role_name || ROLE_LABELS[user.role]) : 'CRM'} <span className="text-gray-400 font-light">Dashboard</span>
                    </h1>
                    {user && (
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400 font-medium flex items-center gap-1">
                    ...
                    System Active &bull; {user?.username ? `Welcome back, ${user.username}` : 'CRM Management'}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-4">
              ...
              >
                ...
                <div className="text-left hidden sm:block">
                  <p className="text-sm font-bold text-gray-800 dark:text-gray-200 transition-colors">{user?.username || "User"}</p>
                  <p className="text-xs text-gray-500 dark:text-gray-400">{user?.role_name || ROLE_LABELS[user?.role] || "Member"}</p>
                </div>
