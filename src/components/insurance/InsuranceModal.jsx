...
const InsuranceModal = ({ isOpen, onClose, onSave, plan, mode }) => {
  ...
  const inputClass = `w-full px-4 py-2 rounded-lg border focus:outline-none focus:ring-2 focus:ring-[var(--color-brand-primary)] bg-white dark:bg-[#022c33] border-gray-300 dark:border-[#45828b]/30 text-gray-900 dark:text-white transition-colors`;

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className={`w-full max-w-md rounded-2xl shadow-2xl bg-white dark:bg-[#033840] text-gray-900 dark:text-white transition-colors`}>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="flex justify-end gap-3 mt-6">
            {isViewMode ? (
              <Button onClick={onClose} className="bg-slate-100 hover:bg-slate-200 text-slate-700 dark:bg-gray-700 dark:text-gray-200 dark:hover:bg-gray-600">Close</Button>
            ) : (
              <>
                <Button onClick={onClose} className="border border-gray-300 hover:bg-gray-50 text-gray-700 dark:border-gray-600 dark:text-gray-300 dark:hover:bg-gray-700 bg-transparent">Cancel</Button>
